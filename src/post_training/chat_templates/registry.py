...
#
# Note on ``qwen3``: byte-identical (modulo spliced ``{% generation %}``
# markers) to the ``chat_template`` field in ``Qwen/Qwen3-8B``'s
# ``tokenizer_config.json``.  The same template ships with every model in the
# original Qwen3 release — 0.6B through 235B-A22B, dense and MoE — so the
# entry is not size-specific.  The ``*-Instruct-2507`` and ``*-Thinking-2507``
# refreshes do carry different templates and would need their own entries.
#
# Unlike the OLMo templates, ``qwen3`` masks *only the final exchange*: Qwen3
# strips ``<think>`` blocks from assistant turns at or before the last user
# query, so history renders as a bare answer with no reasoning trace.
# Training on that teaches the model to answer directly at exactly the
# position where it opens a ``<think>`` block at inference.  The markers
# therefore wrap the assistant body only when
